...
BITBOARD Pawn::getAllMoves() {
	if (color == BLACK)
		return blackForwardMoves[currentPosition] | blackAttackMoves[currentPosition];
	else 
		return whiteForwardMoves[currentPosition] | whiteAttackMoves[currentPosition];
}

BITBOARD Pawn::getForwardMoves() {
	if (color == BLACK)
		return blackForwardMoves[currentPosition];
	else
		return whiteForwardMoves[currentPosition];
}

BITBOARD Pawn::getAttackMoves() {
	if (color == BLACK)
		return blackAttackMoves[currentPosition];
	else 
		return whiteAttackMoves[currentPosition];
}
...
void Pawn::printtmp() {
	...
	std::cout << "#pion 3\n";
	b.printBitboard(whiteForwardMoves[7]);
}
